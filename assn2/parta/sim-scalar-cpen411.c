...
#include "machine.def"
    ...
    for ( i=0; i < 3; ++i ) { // for each potential source operand...
        if ( pI->src[i] != NULL ) {
			...
			// stall when this instruction is a CONDITIONAL BRANCH (or instructions "jr" and "jalr") AND is written to by a LOAD instruction
			// note: this forwarding can only happen when MEM stage of LOAD completes, and forwards the value to this instruction's DECODE stage
			// ie. ld R1, ..
			// 	   bne R1, .., 0x...
			if(((MD_OP_FLAGS(pI->op) & F_COND) || (MD_OP_FLAGS(pI->op) & F_INDIRJMP)) && (MD_OP_FLAGS(pI->src[i]->op) & F_LOAD)){
				if((pI->src[i]->status == FETCHED) || (pI->src[i]->status == DECODED) ||
					(pI->src[i]->status == EXECUTED) || (pI->src[i]->status == MEMORY_STAGE_COMPLETED)){
					pI->stalled = 1;
					return;
				}
			}
						
			// stall when this instruction's source operand is written to by a LOAD instruction (that hasn't completed memory stage)
			// ie. ld R1, ..
			//	   this_inst Rd, R1, ..
			else if(MD_OP_FLAGS(pI->src[i]->op) & F_LOAD){
				if((pI->src[i]->status == FETCHED) || (pI->src[i]->status == DECODED) || (pI->src[i]->status == EXECUTED)){
					
					// for potential STORE instructions, stall ONLY when storing to an effective address that had just been loaded
					// ie. ld r1, ..		AND NO STALLS IF		ld r1, ..
					//	   sw .., 3(r1)								sw r1, ..
					
					// note that "sw ra, off(rb)" corresponds to "sw src[0] src[1]"
					if(!(MD_OP_FLAGS(pI->op) & F_STORE) || (i == 1)){
						pI->stalled = 1;
						return;
					}
				}
			...
			// stall when this instruction is a CONDITIONAL branch (or "jr" and "jalr"), and its source operand is written to by another instruction 
			// note: this forwarding can only happen when pI->src[i] instruction finishes EX stage, and forwards to this instruction's DECODE stage
			// ie. inst R1, .., ..
			//     bne  R1, .., 0x...
			else if((MD_OP_FLAGS(pI->op) & F_COND) || (MD_OP_FLAGS(pI->op) & F_INDIRJMP)){
				if((pI->src[i]->status == FETCHED) || (pI->src[i]->status == DECODED) || (pI->src[i]->status == EXECUTED)){
					pI->stalled = 1;
					return;
				}
...
void sim_main(void)
{
    ...
    do {
        ...
        sim_cycle++; // set breakpoint at this line for gdb
