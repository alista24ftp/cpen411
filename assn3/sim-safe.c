...
#include "machine.def"
	...
	// if instruction is a cond branch
	if(MD_OP_FLAGS(op) & F_COND){
		...
		if(actual_outcome == 1){
			if((bpred_pht[index] & 0x01) != 1){
				bpred_pht[index]++;
			}
		}
		else{
			if((bpred_pht[index] & 0x01) != 0){
				bpred_pht[index]--;
			}
		}
		****/
		...
		int prediction = (bpred_pht[index] & 0x02) >> 1;
		...
		if(actual_outcome == 1){ // taken
			if((bpred_pht[index] & 0x03) != 3){
				bpred_pht[index]++;
			}
		}
		else{
			if((bpred_pht[index] & 0x03) != 0){
				bpred_pht[index]--;
			}
		}
		****/
		...
		int prediction = (bpred_pht[index] & (1 << history)) >> history;
		int actual_outcome = (regs.regs_NPC != (regs.regs_PC + sizeof(md_inst_t))); // taken - 1, not taken - 0
		if( prediction != actual_outcome ) g_total_mispredictions++;
		// update predictor
		if(actual_outcome == 1){
			if(prediction != 1){
				bpred_pht[index] += (1 << history);
			}
		}
		else{
			if(prediction !=0){
		...
		****/
		...
		/**** 2-bit predictor with 4-bit correlation ****
		unsigned index = (regs.regs_PC >> 3) & ((1<<18)-1);
		assert( index < ENTRIES );
		
		int prediction = (bpred_pht[index] >> (history * 2 + 1)) & 0x01;
		int actual_outcome = (regs.regs_NPC != (regs.regs_PC + sizeof(md_inst_t))); // taken - 1, not taken - 0
		if( prediction != actual_outcome ) g_total_mispredictions++;
		// update predictor
		if(actual_outcome == 1){
			if(((bpred_pht[index] >> (history * 2)) & 0x03) != 0x03){
				bpred_pht[index] += (1 << (history * 2));
			}
		}
		else{
			if(((bpred_pht[index] >> (history * 2)) & 0x03) != 0){
				bpred_pht[index] -= (1 << (history * 2));
			}
		}
		// update history
		history = ((history << 1) & 0xf) + actual_outcome;
		****/
		
		/**** GShare predictor ****/
		unsigned index = (history << (TABLE_BITS - CORRELATION_BITS)) ^ ((regs.regs_PC >> 3) & ((1<<18)-1));
		assert( index < ENTRIES );
		...
		// update history
		history = ((history << 1) & (ENTRIES - 1)) + actual_outcome;
		/****/
	}
